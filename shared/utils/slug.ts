 ...
 *
 * The one-shot SQL backfill in the `add_exercise_slug` migration applied the
 * same rules in Postgres. Slugs are write-once: the seed derives
 * `Exercise.slug` from `name` only when it creates a row and never on update,
 * because exercise-media storage paths and deep links are filed under the
 * slug. Changing these rules therefore affects new rows only.
 *
