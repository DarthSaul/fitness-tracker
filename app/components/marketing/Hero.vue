/**
 * Split hero: copy and CTAs on the left, the sign-in artwork framed on the
 * right.
 *
 * The art is a 940px-wide portrait, so it is used near its native aspect
 * inside a 440px frame — one 1x and one 2x source, no upscaling. Bleeding it
 * full-width across the frame, as the login screen does, would visibly soften
 * it on a retina laptop.
 *
 * The card is the app's own AppCard with its brand rail, so the marketing page
 * opens on the same object the product is built from.
 */
...
<template>
  <section class="mx-auto w-full max-w-frame px-6 py-16 lg:px-10 lg:py-24">
    <div class="grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,440px)] lg:gap-12">
      <div class="max-w-2xl">
        ...
        <p class="mt-5 text-body text-label-secondary sm:text-title2 sm:font-normal">
          A workout tracker for people following a real program — weeks, days and
          progressions already laid out. Pick one, log your sets, and let it
        ...
        <p v-if="appleEnabled" class="mt-3 text-footnote text-label-secondary">
          Sign in with Apple is available too.
        </p>
